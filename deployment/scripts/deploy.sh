...
sudo /bin/bash ./PythonSyntheaFHIRClient/deployment/scripts/install_python_daemon.sh $connection_string $polling_interval $FHIR_output_path $local_output_path $log_path $container_name
sudo mv ./PythonSyntheaFHIRClient/deployment/scripts/deploy_config.json ./PythonSyntheaFHIRClient/python_client/
echo "Done!"
